...
<c:if test="${sort=='popular'}">
	<c:forEach var="v" items="${popPList}" begin="0" end="14"
		varStatus="status">
		<c:set var="img" value="${v.imageNames}"></c:set>
				...
				<div class="featured__item__pic set-bg" data-setbg="${img[0]}">
					<ul class="featured__item__pic__hover">
						<li><a href="javascript:void(0);"
							onclick="javascript:insertCart(${v.pNum});"> <i
				...
				<div class="product__discount__item">
					...
					<div class="product__discount__item__text">
						...
						<div class="product__item__price">
							<fmt:formatNumber value="${v.dc_price}" pattern="#,###" />
							원<span><fmt:formatNumber value="${v.price}"
									pattern="#,###" />원</span>
						</div>
					</div>
				</div>
...
<c:if test="${sort=='related'}">
	<c:forEach var="v" items="${pList}" begin="0" end="9"
		varStatus="status">
		<c:set var="img" value="${v.imageNames}"></c:set>
...
<c:if test="${sort=='adminhome'}">
	<c:forEach var="v" items="${oList}" begin="0" end="5"
		varStatus="status">
		<li class="d-flex mb-4 pb-1">
			<div class="avatar flex-shrink-0 me-3">
				<c:if test="${v.oPay=='카카오페이'}">
					<img src="assets/img/icons/unicons/kakaopay.png" alt="User"
						class="rounded" />
				</c:if>
				<c:if test="${v.oPay=='카드결제'}">
					<img src="assets/img/icons/unicons/card.png" alt="User"
						class="rounded" />
				</c:if>
				<c:if test="${v.oPay=='휴대폰결제'}">
					<img src="assets/img/icons/unicons/phone_pay.png" alt="User"
						class="rounded" />
				</c:if>
			</div>
			<div
				...
				<div class="blog__item__text">
					<ul>
						...
						<li><img style="width: 20px;" src="img/eyes.png"
							alt="조회수 아이콘"> ${v.boardView }</li>
						<c:choose>
							<c:when test="${v.isChecked==true}">
								<li id="${v.boardNum}"><a href="javascript:void(0);"
									onclick="javascript:updateLike(${v.boardNum}, down);"> <img
										class="heartImg" style="width: 20px; cursor: pointer;"
										src="img/fullheart.png" alt="좋아요 아이콘">
								</a> ${v.likeCnt}</li>
							</c:when>
							<c:otherwise>
								<li id="${v.boardNum}"><a href="javascript:void(0);"
									onclick="javascript:updateLike(${v.boardNum}, up);"> <img
										class="heartImg" style="width: 20px; cursor: pointer;"
										src="img/heart.png" alt="좋아요 아이콘">
								</a> ${v.likeCnt}</li>
							</c:otherwise>
						</c:choose>
					</ul>
					...
					<a href="selectOneBoard.do?boardNum=${v.boardNum}"
						class="blog__btn"> 글 보기<span class="arrow_right"></span>
					</a>
				</div>
			</div>
		</div>
	...
	<c:forEach var="v" items="${replyset}" begin="0" varStatus="status">
			...
			<div id="replyContent">
				<c:choose>
					<c:when test="${reply.status==3}">
						<div
							style="font-size: 15px; color: #6f6f6fc4; margin-bottom: 10px;"
							id="replyContent">삭제된 댓글입니다.</div>
					</c:when>
					<c:otherwise>
						<ul style="font-size: 13px; float: left; display: flex;"
							id="replyInfo">
							<li style="margin-right: 4px;"><i class="fa fa-calendar-o"></i>
								${v.replyDate}</li>
							<li>
								<button id="showReReply"
									style="border-radius: 10px; border: 1px solid #b2b2b2; margin-left: 10px; font-size: 12px;">답글
									달기</button>
							<li>
		...
		<div id="rereplywrite">
			<form action="#" method="post" style="width: 90%;">
				<ul>
					<li
						style="float: left; list-style: none; width: 80%; margin: 10px;"><textarea
							style="border-radius: 5px; border: 1.7px solid #6667ab6b; width: 100%; height: 50px;"
							name="reply" placeholder="댓글을 작성하세요" required></textarea></li>
					<li style="float: left; list-style: none; margin-left: -10px;"><input
						style="border: 1px solid #6667ab42; float: right; color: white; padding: 10px; border-radius: 5px; background-color: #6667AB; margin: 10px; width: 80%;"
						type="submit" value="댓글 작성"></li>
				</ul>
				<br>
			</form>
		</div>
