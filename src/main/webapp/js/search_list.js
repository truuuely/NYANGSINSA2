...
let searchContent; // 목록 요청 매개변수(카테고리)
...
function list(searchContentIn, selectPageIn) {
	searchContent = searchContentIn;
...
function paging(currentPage) {
	if (totalData != 0) { // dataList에 데이터(상품||리뷰)가 있을 때 페이징 띄우기
		...
		if (first > 1) {
			pageHtml += "<a href='javascript:list(`" + searchContent + "`,`" + (first - 1) + ")' id='prev'><li> ◀ </li></a>";
		}
		...
		for (var i = first; i <= last; i++) {
			if (currentPage == i) {
				pageHtml +=
					"<a href='javascript:list(`" + searchContent + "`,`" + i + ")' id='" + i + "'><li class='on'>" + i + "</li></a>";
			} else {
				pageHtml += "<a href='javascript:list(`" + searchContent + "`," + i + ")' id='" + i + "'><li>" + i + "</li></a>";
			}
		}

		if (last < totalPage) {
			pageHtml += "<a href='javascript:list(`" + searchContent + "`," + (last + 1) + ")' id='next'><li> ▶ </li></a>";
		}
...
/*// 가격 범위 선택 부분
function lowPrice(val){
   console.log("minamount");
   lowPrice=val;
   list(category,sort,1,lowPrice,highPrice);
}

function highPrice(val){
