...
public class ProductDAO {
	...
	// 인기 상품 검색
	private final String SQL_SELECTALL_POPULAR = "SELECT PRODUCT.P_NO, P_NM, P_PRICE, P_CATEGORY, SUM(OD_CNT), DC_PERCENT, P_AMT, P_DETAIL, "
			+ " FROM PRODUCT INNER JOIN ORDER_DETAIL ON ORDER_DETAIL.P_NO = PRODUCT.P_NO WHERE PRODUCT.P_PRICE BETWEEN ? AND ? "
			+ " GROUP BY PRODUCT.P_NO, P_NM, P_PRICE, P_CATEGORY, DC_PERCENT, P_AMT, P_DETAIL,ORDER BY SUM(OD_CNT) DESC";
	
	// 상품 카테고리별 인기순
	final String SQL_SELECTALL_CATEGORY_SELLDESC = "SELECT PRODUCT.P_NO, P_CATEGORY, P_NM, P_PRICE, DC_PERCENT, P_AMT, "
			+ " P_DETAIL, P_SUM(OD_CNT) FROM ORDER_DETAIL INNER JOIN PRODUCT ON ORDER_DETAIL.P_NO = PRODUCT.P_NO AND P_CATEGORY= ?"
			+ " WHERE PRODUCT.PRICE  BETWEEN ? AND ? GROUP BY PRODUCT.P_NO, P_CATEGORY, P_NM, P_PRICE, "
	...
	public ArrayList<ProductVO> selectAll(ProductVO pvo) {
		...
		else if(ivo.getTypeNum()==101||ivo.getTypeNum()==102) {
