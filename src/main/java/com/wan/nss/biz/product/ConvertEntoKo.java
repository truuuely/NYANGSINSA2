...
public class ConvertEntoKo {
	...
	public String engToKor(String eng) {
		...
		for (int i = 0; i < eng.length(); i++) {
			...
			System.out.println((char) (0xAC00 + initialCode ));
			System.out.println((char) (0xAC00 + initialCode + medialCode));
			System.out.println((char) (0xAC00 + initialCode + medialCode + finalCode));
			System.out.println("---------------------");
			// 추출한 초성 문자 코드, 
	...
	private int getCode(CodeType type, String c) {
		...
		case chosung:
			...
			System.out.println(c + "의 인덱스 : " + index);
			System.out.println(init.length());
			
			if (index != -1) { // 초성 목록 중에 있으면
				return index * 21 * 28;
			}
			break;
		case jungsung:
